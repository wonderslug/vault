...
func NewTestCluster(t testing.TB, base *CoreConfig, unsealStandbys bool) *TestCluster {
	testCluster := &TestCluster{
		ReloadFuncs:     make([]*map[string][]reload.ReloadFunc, 0, 3),
	...
	err = ioutil.WriteFile("ca_cert.pem", testCluster.CACertPEM, 0755)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = ioutil.WriteFile("ca_key.pem", testCluster.CAKeyPEM, 0755)
	if err != nil {
		t.Fatal(err)
	}
	...
	certGetter := reload.NewCertificateGetter(s1CertFile, s1KeyFile)
	...
	certGetter = reload.NewCertificateGetter(s2CertFile, s2KeyFile)
	...
	certGetter = reload.NewCertificateGetter(s3CertFile, s3KeyFile)
